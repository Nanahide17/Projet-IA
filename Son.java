...
public class Son {
	...
	public Son(final String nomFichier) {
		try {
				...
				af.getSampleSizeInBits() == 16) // et que les échantillons sont sur 16 bits
			{
				...
				final byte[] bufferOctets = new byte[NombreDonnees]; // Préparer un buffer pour lire tout le flux du
																		// fichier
				...
				ShortBuffer donneesAudio = bb.asShortBuffer(); // Préparer un buffer pour interpréter les données en
																// tableau de short
	...
	public static void main(String[] args) {
		if (args.length == 1) {
			...
			for (int i = 0; i*4096 <= son.donnees().length; i++) {
				System.out.println("Bloc" + i + " : " + son.bloc_deTaille(1, 4096).length + " échantillons à "
						+ son.frequence() + "Hz");
			}
		} else
