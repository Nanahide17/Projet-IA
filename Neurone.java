public abstract class Neurone implements iNeurone
{
	...
	public int apprentissage(final float[][] entrees, final float[] resultats)
	{
		...
		// Créer un "drapeau" indiquant si toutes les entrées ont permis de trouver les résultats attendus (=> l'apprentissage est alors fini), ou s'il y a au moins un cas qui ne correspond pas (=> apprentissage pas fini)
		boolean check=false;
		...
		while(!check){
			...
			//for(float entree : entrees){
				...
				float difference=resultats[i]-sortie();
				// Si l'erreur absolue dépasse la tolérance autorisée 
				if(difference>ToleranceSortie || difference<-ToleranceSortie){
					// On met à jour les poids synaptiques
					float[] entries=entrees[i];
					for(int j=0; j < entries.length; j++){
						float temp=synapses()[j];
							synapses()[j]=temp+eta*entries[j]*(resultats[i]-sortie());
					}
